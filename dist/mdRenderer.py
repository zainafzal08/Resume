...
plsRenderMd = m.Markdown(HighlighterRenderer(), extensions=('fenced-code',))

def manualReplace(s, r, t):
    l = len(s)
...
def additionalRender(text):
    ...
    while s:
        ...
        quote += "</blockquote>"
        text = manualReplace(s.group(0),quote,text)
        s = re.search('<blockquote>[\n\s]*<p>[\n\s]*(.*)[\n\s]*</p>[\n\s]*</blockquote>',text)
    return text
def render(inputText):
    return additionalRender(plsRenderMd(inputText))
